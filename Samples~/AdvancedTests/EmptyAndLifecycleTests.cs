...
namespace Blobcheg.AdvancedTests
{
    ...
    public sealed class EmptyAndLifecycleTests : AdvancedFixture
    {
        ...
        // BUG: две сырые записи нулевой длины получают ОДИН И ТОТ ЖЕ оффсет.
        // Ожидалось: у двух разных записей два разных адреса — адрес и есть единственная личность
        // записи в этом формате, и совпасть они не могут.
        // Корень: BlobchegWriter.Layout двигает position на Bytes.Length ПОСЛЕ выдачи оффсета, и при
        // нулевой длине position не двигается вовсе; следующий AlignUp возвращает то же значение.
        // Наружу это выходит в BlobchegBuild.Upsert: ref-ассеты двух разных нод получают равный
        // offset, и потребитель, читающий по каждому из них, читает одни и те же байты.
        [Test]
        ...
        // BUG: копия структуры базы переживает Dispose оригинала и считает себя поднятой.
        // Ожидалось: освобождённая база не должна считаться поднятой НИ ЧЕРЕЗ ОДИН свой экземпляр —
        // иначе вторая копия это заряженный use-after-free, а второй Dispose по ней — двойное
        // освобождение и падение процесса без стека.
        // Корень: BlobchegBlob/BlobchegBuffer — обычные struct'ы с владеющим указателем, а Dispose
        // зануляет Ptr только в СВОЁМ экземпляре. Копия делается любым присваиванием, передачей в
        // джобу или укладкой в поле системы; версии владения (safety handle, как у NativeArray) в
        // буфере нет намеренно, и отличить живую копию от мёртвой снаружи нечем.
        [Test]
        public void Копия_освобождённой_базы_не_обязана_считать_себя_поднятой()
        {
            Node<AdvComboNodeSo>("Combo");
            Rebuild();
            ...
            Assert.That(copy.IsCreated, Is.False,
                "копия висит на освобождённой памяти — узнать об этом снаружи обязано быть возможно");

            // Читать через copy здесь НЕЛЬЗЯ: это чтение освобождённой памяти, оно уронит редактор
            // вместе с отчётом. Тест смотрит только на то, что копия о своей смерти не знает.
        }
