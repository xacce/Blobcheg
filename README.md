...
Часть тестов там КРАСНАЯ намеренно — это находки, а не поломка набора; над каждой стоит
комментарий `// BUG:` с корнем. Ставить через Package Manager → Samples → Import либо гонять из
CLI:
